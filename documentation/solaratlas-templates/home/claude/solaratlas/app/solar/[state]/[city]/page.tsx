...
const CITIES: Record<string, CityData> = {
  phoenix: {
    id: 1, name: 'Phoenix', slug: 'phoenix', state: 'Arizona', stateSlug: 'arizona', stateAbbr: 'AZ',
    solarScore: 98, peakSunHours: 6.5, annualKwhPerKw: 1800, avgElectricRate: 0.13,
    annualSavings: 936, paybackYears: 12.8, savings25Year: 23400,
    systemCost4kw: 12000, federalTaxCredit: 3600, stateIncentives: 2000, netCost: 6400,
    ...
    cloudyDaysPerYear: 85, rainyDaysPerYear: 36, installerCount: 127,
    climateType: 'Hot Desert', avgTemperature: 75, humidity: 18,
    nearByCities: ['Tucson', 'Mesa', 'Scottsdale', 'Tempe', 'Las Vegas'],
    topInstallers: ['Sunrun', 'SunPower', 'Tesla Energy', 'Palmetto', 'Sunnova'],
    metaTitle: 'Solar Potential in Phoenix, Arizona | SolarAtlas',
    metaDescription: 'Phoenix solar: 98/100 score. Save $936/year. 6.5 peak sun hours. Free interactive calculator.',
  },
  austin: {
    id: 3, name: 'Austin', slug: 'austin', state: 'Texas', stateSlug: 'texas', stateAbbr: 'TX',
    solarScore: 86, peakSunHours: 5.4, annualKwhPerKw: 1480, avgElectricRate: 0.12,
    annualSavings: 696, paybackYears: 17.2, savings25Year: 17400,
    systemCost4kw: 12000, federalTaxCredit: 3600, stateIncentives: 1500, netCost: 6900,
    ...
    cloudyDaysPerYear: 104, rainyDaysPerYear: 90, installerCount: 89,
    climateType: 'Subtropical', avgTemperature: 68, humidity: 60,
    nearByCities: ['San Antonio', 'Dallas', 'Houston', 'Waco', 'College Station'],
    topInstallers: ['Sunrun', 'Tesla Energy', 'Freedom Solar', 'Momentum Solar', 'Pink Energy'],
    metaTitle: 'Solar Potential in Austin, Texas | SolarAtlas',
    metaDescription: 'Austin solar: 86/100 score. Save $696/year. 5.4 peak sun hours. Free calculator.',
  },
}
...
export default function CityPage() {
  ...
  const citySlug = (params?.city as string) || 'phoenix'
  const city = CITIES[citySlug] || CITIES['phoenix']
  ...
  const calcAnnualOutput = systemKw * city.annualKwhPerKw
  ...
  const calcPayback = calcAfterCredit > 0 ? (calcAfterCredit / calcAnnualSavings).toFixed(1) : '—'
  const calcMonthlyPayment = Math.round(calcAfterCredit / 120)

  // Chart data
  const monthlyChartData = MONTHS.map((m, i) => ({
    month: m,
    kWh: Math.round((city.monthlyKwh[i] / 4) * systemKw),
    sunHours: city.monthlySunHours[i],
    savings: Math.round((city.monthlyKwh[i] / 4) * systemKw * city.avgElectricRate),
  }))
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {city.nearByCities.map(nearCity => {
              const adjustedScore = Math.max(70, city.solarScore - Math.floor(Math.random() * 10))
              const adjustedSavings = Math.round(city.annualSavings * (0.85 + Math.random() * 0.3))
              return (
                <a
                  key={nearCity}
                  href={`/solar/${city.stateSlug}/${nearCity.toLowerCase().replace(/\s+/g, '-')}`}
                ...
                >
                  <div className="font-semibold group-hover:text-emerald-400 transition text-sm">{nearCity}</div>
                  <div className="text-emerald-400 text-xl font-bold mt-1">{adjustedScore}</div>
                  <div className="text-xs text-slate-400">score · ${adjustedSavings}/yr</div>
                </a>
              )
            })}
          </div>
      ...
      <section className="px-6 py-16">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-2xl font-bold mb-8">What {city.name} Residents Say</h2>
          <div className="grid md:grid-cols-3 gap-5">
            {[
              { name: 'Sarah M.', since: '2023', savings: city.annualSavings, quote: `Our electric bill went from $${monthlyBill} to almost nothing. Best decision we've made for the house.` },
              { name: 'James R.', since: '2022', savings: Math.round(city.annualSavings * 1.1), quote: `Took ${city.paybackYears} years to break even but now we're pure profit. ${city.name} gets so much sun it was a no-brainer.` },
              { name: 'Lisa T.', since: '2024', savings: Math.round(city.annualSavings * 0.9), quote: `The installer quotes came in $3,000 apart — glad I compared. Use this site and check at least 3.` },
            ].map(t => (
              <div key={t.name} className="bg-slate-800 rounded-2xl p-6">
                ...
                <div className="flex justify-between text-sm">
                  <div>
                    ...
                    <div className="text-slate-400 text-xs">{city.name} homeowner since {t.since}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-emerald-400 font-bold">${t.savings.toLocaleString()}/yr</div>
